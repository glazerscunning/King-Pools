...
class A_PayPal_Express_Checkout_Button extends Mixin
{
	...
	{
		return $this->render_partial('photocrati-paypal_express_checkout#button', array(
	...
	function _paypal_request($method, $data)
	{
		...
		if (defined('NGG_PRO_PAYPAL_LIVE_URL')) $url = NGG_PRO_PAYPAL_LIVE_URL;
	...
	function create_paypal_express_order()
	{
		$retval = FALSE;
        $settings = C_NextGen_Settings::get_instance();
		...
		if (isset($_REQUEST['token'])) {
			...
			if (isset($response['email'])) {
                ...
                for ($i=0; isset($response["l_number".$i]); $i++) {
                    ...
                    $images['images'][$image_id]['items'][$item_id] = array(
                        'quantity'	=>	intval($response['l_qty'.$i]),
                        'price'		=>	doubleval($response['l_amt'.$i]),
                    );
                }
                ...
                $order = $this->create_order(
                    ...
                    $response['shiptostreet'],
                    $response['shiptocity'],
                    $response['shiptostate'],
                    $response['shiptozip'],
                    $response['shiptocountryname'],
					$response['custom']
                );
				...
				if (($retval = $order->save())) {
					...
					if (is_array($response) && isset($response['ack']) && $response['ack'] == 'Success') {
                        if (!isset($order->paypal_data)) $order->paypal_data = array();
						$order->paypal_data = array_merge($order->paypal_data, $response);
						$order->gateway_admin_note = __('Payment was successfully made via PayPal Express Checkout, with no further payment action required.');
	...
	{
		$response = $this->_paypal_request('GetExpressCheckoutDetails', array(
			'TOKEN'	=>	$token
		));
	...
	function set_express_checkout()
	{
		...
		$data = array(
			...
			'CALLBACKTIMEOUT'					=> 6,
			'NOSHIPPING'						=> 0, # use 1 if the cart only contains digital downloads
		...
		if ($settings->paypal_page_style) $data['PAGESTYLE'] = $settings->paypal_page_style;
			...
			foreach ($cart_items as $image_id => $items) {
				if (($image = $image_mapper->find($image_id))) {
					...
					foreach ($items as $item_id => $quantity) {
                        ...
                        else $data['NOT_FOUND'.$item_number] = $item_id;
					}
        ...
        if (isset($response['l_longmessage0'])) $response['error'] = $response['l_longmessage0'];
