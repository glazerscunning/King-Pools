(function($) {
    ...
    var ngg_image_proofing = {
        ...
        textList: {
            image_list: 'Submitting {0} image{1}',
            submit_cancel: 'Cancel',
            submit_button: 'Send',
            submit_message: 'Submitting...',
            example_name: 'Name',
            example_email: 'Email'
        },
        ...
        getDialog: function(gallery_id) {
            ...
            if (dialog.length == 0) {
                var proofed_list = self.getList(gallery_id);
                var imgcount = self.formatString.call(self.textList.image_list, proofed_list.length, proofed_list.length != 1 ? 's' : '');
                var disabled = '';
                if (proofed_list.length <= 0) {
                ...
                tmp += '<input type="text" required class="ngg-proofing-fullname" name="customer_name" placeholder="' + self.textList.example_name+ '"/>';
                tmp += '</div>';

                tmp += '<div class="ngg-proofing-email-wrapper">';
                tmp += '<span><i class="fa fa-fw fa-envelope"></i></span>';
                tmp += '<input type="email" required class="ngg-proofing-email" name="email" placeholder="' + self.textList.example_email + '"/>';
                tmp += '</div>';

                tmp += '</div>';
                tmp += '<div class="ngg-proofing-form-entry ngg-form-entry-submit">';
                tmp += '<button class="ngg_pro_btn ngg-proofing-submit" type="submit" name="submit"' + disabled + '>' + self.textList.submit_button + '&nbsp;<i class="fa fa-arrow-circle-right"></i></button>';
                tmp += '</div>';
                tmp += '</div></div>';
                dialog = $(tmp);
                ...
                dialog.find('.ngg-proofing-submit').click(function(event) {
                    ...
                    submitbtn.html(self.textList.submit_message);
                    ...
                    $.post(
                        ...
                        function(data) {
                            ...
                            submitbtn.html(self.textList.submit_button);
        ...
        updateAllGalleries: function() {
            ...
            $.each(galleries, function(index, gallery) {
                self.setActiveTriggers(gallery.transient_id);
            });
        },
        ...
        setDialogStrings: function(dialog, proofed_list) {
            ...
            label.html(self.formatString.call(self.textList.image_list, proofed_list.length, proofed_list.length != 1 ? 's' : ''));
