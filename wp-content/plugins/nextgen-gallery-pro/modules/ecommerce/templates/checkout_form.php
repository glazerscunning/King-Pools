...
<script type="'text/template" id="ngg_pro_cart_item_tmpl">
    <td class='quantity_column'>
        <input type='number' min='0' name='items[{item.image_id}][{item.id}]' value='{item.quantity}' class='quantity_field'/>
...
<form id="ngg_pro_checkout" action="<?php echo $_SERVER['REQUEST_URI']?>" method="post">
    <div id="ngg_pro_links_wrapper">
        <?php if ($referrer_url): ?>
            <a class='ngg_pro_btn' href="<?php esc_attr_e($referrer_url)?>" id="ngg_pro_continue_shopping"><?php esc_html_e($i18n->continue_shopping)?></a>
        <?php endif ?>
