...
class A_Stripe_Checkout_Button extends Mixin
{
	...
	{
		$i18n = new stdClass;
		$i18n->pay_with_card = __('Pay with Card', 'nggallery');
		return $i18n;
	}
	...
	function get_stripe_vars($include_private_key=FALSE)
	{
		...
		$retval = array(
            ...
            'key'               =>  $settings->ecommerce_stripe_key_public,
            'currency'          =>  C_NextGen_Pro_Currencies::$currencies[$settings->ecommerce_currency]['code'],
            'shippingAddress'   =>  TRUE,
		);

        if ($include_private_key) $retval['private_key'] = $settings->ecommerce_stripe_key_private;
	...
	{
		return $this->render_partial('photocrati-stripe#button', array(
			'i18n'			=>	$this->get_i18n_strings(),
			'stripe_vars'	=>	json_encode($this->object->get_stripe_vars())
    ...
    function create_stripe_charge()
    {
        ...
        if (!class_exists('Stripe')) include_once('stripe-sdk/lib/Stripe.php');
        ...
        if (($stripe = $this->param('stripe')) && isset($stripe['token']) && $this->param('items')) {
            ...
            foreach ($req_fields as $field) if (!isset($stripe[$field])) $missing_fields[] = $field;
            if ($missing_fields) {
            ...
            // Create an order
                ...
                $order = $this->create_order(
                    ...
                    $total,
                    'stripe_checkout',
                    $stripe['shipping_street_address'],
                    $stripe['shipping_city'],
                    $stripe['shipping_state'],
                    $stripe['shipping_zip'],
                ...
                try {
                    $charge_params = array(
                        ...
                        'metadata'      =>  array(
                            'order_id'  =>  $order->ID(),
                            'description'   =>  sprintf(__('Order from %s for %s (%s)'), $stripe['site_name'], $stripe['customer_name'], $stripe['email']),
                        )
                    );
