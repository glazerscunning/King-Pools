...
Galleria.get_npl_setting = function(name, def) {
    var tmp = '';
    if (typeof window.lightbox_settings != 'undefined' && window.lightbox_settings[name] != '') {
...
Galleria.get_show_setting = function() {
    var retval = 0;
    $.each(window.Galleria_Instance.fetch_images(), function(index, element) {
        ...
        var methods = {
            sidebar: {
                ...
                events: {
                    ...
                    npl_init: function() {
                        ...
                        self.$('sidebar-container').css({background: Galleria.get_npl_setting('sidebar_background_color')});
            ...
            sidebars: {
                comments: {
                    ...
                    request: function(id, page, finished) {
                        ...
                        if (Galleria.get_npl_setting('lang', false)) {
                            postdata.lang = Galleria.get_npl_setting('lang');
                        }
            ...
            thumbnails: {
                ...
                events: {
                    ...
                    npl_init: function() {
                        self.$('thumbnails-container').addClass('opened');
                        self.$('thumbnails-container').css({background: Galleria.get_npl_setting('carousel_background_color')});
                        ...
                        var dock_toggle_container = self.$('dock-toggle-container')
                            .css({background: Galleria.get_npl_setting('carousel_background_color')});
                        var dock_toggle_button = $('<i/>').addClass('fa fa-angle-down')
                            .css({color: Galleria.get_npl_setting('carousel_text_color')});
            ...
            info: {
                ...
                events: {
                    ...
                    npl_init: function() {
                        ...
                        self.$('info, info-text, info-title, info-description').css({background: Galleria.get_npl_setting('carousel_background_color')});
                    },
                    _image_ran_once: false,
                    image: function() {
            ...
            icons: {
                get_color: function() {
                    return Galleria.get_npl_setting('icon_color');
                },
                get_background: function() {
                    var iconcolor = Galleria.get_npl_setting('carousel_background_color');
                    if (Galleria.get_npl_setting('icon_background_enabled', false)) {
                        iconcolor = Galleria.get_npl_setting('icon_background');
                    }
                    return iconcolor;
                },
                get_overlay_color: function() {
                    return Galleria.get_npl_setting('overlay_icon_color');
                }
            },
            nplModal: {
                close: function() {
                ...
                events: {
                    bind: function() {
                        ...
                        if (Galleria.get_npl_setting('image_protect', false)) {
            ...
            galleria: {
                get_npl_setting: function(name, def) {
                    return Galleria.get_npl_setting(name, def);
                },
                get_displayed_gallery_setting: function(name, def) {
                    var tmp = '';
                    if (typeof top.galleries['gallery_' + window.Galleria_Instance.displayed_gallery.ID].display_settings[name] != 'undefined') {
                        tmp = top.galleries['gallery_' + window.Galleria_Instance.displayed_gallery.ID].display_settings[name];
                    } else {
                        tmp = def;
                    }
                    if (tmp == 1) tmp = true;
                ...
                events: {
                    ...
                    npl_init: function() {
                        ...
                        self.$('container').css({background: Galleria.get_npl_setting('background_color')});
                    ...
                    loadfinish: function() {
                        ...
                        if (!methods.galleria.events._loadfinish_ran_once) {
                            ...
                            &&  !window.Galleria_Instance.ajax_image_fetch_completed) {
                                var showsetting = Galleria.get_show_setting();
                                $.ajax({
                                    async: true,
                                    ...
                                    success: function(data, status, jqXHR) {
                                        ...
                                        $.each(data, function(ndx, newimage) {
                                            ...
                                            if (!found) {
                                                self.push(newimage);
                                                // check if the image-to-show was loaded by ajax
                                                if (newimage.image_id == top.nplModalRouted.image_id && showsetting == 0) {
                                                    setTimeout(function() {
                                                        self.show(methods.galleria.get_index_from_id(newimage.image_id));
                                                    }, 50);
                                                }
                                            }
                                        });
